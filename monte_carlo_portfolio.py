...
distribution, sampled in random blocks. Each sampled path is shifted in
log-space so the long-run CAGR matches target_equity_cagr. This preserves
the real shape of history (volatility, crash depth, sequence of returns)
while applying a forward-looking assumption that discounts the unrepeatable
US-superpower premium.
...
# Raw historical geometric means (informational only — paths are shifted to targets at runtime)
...
DEFAULT_TARGET_EQUITY_CAGR = 0.07   # 7% nominal — ~4% real (DMS forward-looking)
DEFAULT_TARGET_INFLATION   = 0.03   # 3% nominal
...
def _shift_to_target(series: List[float], target_cagr: float) -> List[float]:
    """Shift a return series in log-space so its geometric mean equals target_cagr.

    Preserves the volatility profile and sequence of returns from history
    while applying a forward-looking long-run assumption.
    """
    if not series:
        return series
    raw_log_mean = fmean(math.log1p(x) for x in series)
    target_log_mean = math.log1p(target_cagr)
    delta = target_log_mean - raw_log_mean
    return [max(math.exp(math.log1p(v) + delta) - 1.0, -0.9999) for v in series]
...
def bootstrap_historical_series(
    ...
    """Stitch random historical blocks, then shift to configured CAGR targets."""
    ...
    return (
        _shift_to_target(eq_raw,  cfg.target_equity_cagr),
        _shift_to_target(inf_raw, cfg.target_inflation),
    )
...
def simulate_path(cfg: SimulationConfig, seed: int) -> SimulationPath:
    rng = random.Random(seed)
    equity_series, inflation_series = bootstrap_historical_series(cfg, rng)
    ...
    for age, equity_return, inflation in zip(ages, equity_series, inflation_series):
        ...
        # --- Portfolio growth ---
        portfolio_return = cfg.equity_allocation * equity_return + (1.0 - cfg.equity_allocation) * bond_return
        ...
        # --- Social Security (starts at configured age, grows with CPI thereafter) ---
        ...
        if age >= cfg.retirement_age:
            ...
            # --- Fund wedge at the start of retirement ---
            ...
            # --- Compute spending (with volatility adjustment) ---
            total_assets_before_spending = portfolio_wealth + wedge_cash
            actual_spending = build_withdrawal(
                base_spending,
                equity_return,
                cfg.target_equity_cagr,
                cfg.spending_volatility,
            )
            ...
            base_spending, actual_spending, reference_withdrawal_rate = apply_guardrail_cut(
                base_spending,
                actual_spending,
                total_assets_before_spending,
                reference_withdrawal_rate,
            )
            ...
            # --- Net spending after SS offsets it ---
            ...
            if not wedge_retired:
                ...
                current_portfolio_wr = (
                    ...
                    if total_assets > 0.0 and net_spending_need > 0.0
                    else 0.0
                )

                # Escape: portfolio has grown enough that wedge is no longer needed
                if (
        ...
        # --- Depletion detection (before clamping to zero) ---
        shortfall = max(0.0, -(portfolio_wealth + wedge_cash))
        year_depleted = age >= cfg.retirement_age and shortfall > 0.0
        if year_depleted:
        ...
        # SS-only: assets gone but SS alone covers this year's spending
        if (
...
def compute_percentile_rows(results_by_age: Dict[int, List[float]]) -> Dict[int, Dict[str, float]]:
    ...
    for age, values in results_by_age.items():
        out[age] = {label: percentile(values, pct) for label, pct in labels}
    return out
...
def run_simulation(
    ...
    for seed in range(cfg.simulations):
        ...
        if path.depleted_any_time:
            depleted_any_time_count += 1
        if path.terminal_depleted:
            terminal_depleted_count += 1
        if path.ss_only_any_time:
            ss_only_any_time_count += 1
        if path.wedge_depleted:
...
def print_summary(
    ...
    print("=" * 75)
    print("MONTE CARLO SIMULATION SUMMARY — Raw Historical Block Bootstrap")
    print("=" * 75)
    ...
    print()
    header_left = f"PORTFOLIO AT AGE {end_age}"
    print(f" {header_left:<30}{'NOMINAL':>18}{'TODAY\'S $':>18}")
    print("-" * 75)
    for label, pct in [
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print("Running Monte Carlo simulation (raw historical block bootstrap)...")
    print()
    ...
    (
        ...
        matrix,
        percentile_rows,
        depleted_any_time_rate,
        terminal_depleted_rate,
        ss_only_any_time_rate,
    ...
    print(f"Percentiles saved to: {pct_csv}")
    print()
    print_summary(
        cfg,
        ages,
